...
- **Dev Komutu:**
  ...
  ```
  *Bu komut hem Express backend sunucusunu hem de Vite frontend dev sunucusunu ve Electron penceresini anında sıcak yenileme (hot reload) ile çalıştırır.*
...
3. **Electron Paketleme ve GitHub Release Oluşturma:**
   ...
   npm run build:exe
   ```
   *Bu komut `electron-builder` çalıştırarak Windows `.exe` çıktısını üretir ve GitHub `minikkerata/media_planner` deposuna otomatik bir Release taslağı/yayınlaması yükler.*
...
| Komut | Açıklama |
| :--- | :--- |
| `npm run dev` | Web geliştirme modunda çalıştırma |
| `npm run electron:dev` | Electron geliştirici penceresinde anında yenileme ile çalıştırma |
| `npm run build` | Frontend React üretici derlemesi yapma |
| `npm run build:exe` | Taşınabilir ve Kurulumlu Windows `.exe` çıktısı alma |
